...
import { getProjectCategoryById, updateProjectCategory, updateProjectCategoryHiddenStatus } from "/api/projectCategoryServices"
...
export default function ProjectCategoryDetails() {
    ...
    const handleUpdateStatus = async () => {
        try {
            const response = await updateProjectCategoryHiddenStatus(params.id, formData?.isHidden ? false : true);
            console.log(response);
            toast.success("Cập nhật thành công!");
        ...
        <PageContainer title={formData.name} description="Chi tiết phân loại dự án">
            <DashboardCard>
                <DetailsPage
                    ...
                    deleteMessage={formData?.isHidden ? "Hiện phân loại dự án này?" : "Ẩn phân loại dự án này?"}
